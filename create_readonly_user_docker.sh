...
if ! psql -U "$DB_ADMIN_USER" -d "$DB_NAME" -c "SELECT 1;" > /dev/null 2>&1; then
...
USER_EXISTS=$(psql -U "$DB_ADMIN_USER" -d postgres -tAc "SELECT 1 FROM pg_roles WHERE rolname='$READONLY_USER'")

if [ "$USER_EXISTS" = "1" ]; then
    echo -e "${YELLOW}Пользователь '$READONLY_USER' уже существует.${NC}"
    echo "Удаление существующего пользователя..."
    psql -U "$DB_ADMIN_USER" -d postgres <<EOF
...
psql -U "$DB_ADMIN_USER" -d "$DB_NAME" <<EOF
